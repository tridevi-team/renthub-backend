...
export async function up(knex: Knex): Promise<void> {
    return knex.raw(`
        ...
        VALUES (NEW.id, NEW.room_id, NEW.contract_id, NEW.deposit_amount, NEW.deposit_status, NEW.deposit_date, NEW.deposit_refund, NEW.deposit_refund_date, NEW.rental_start_date, NEW.rental_end_date, NEW.status, "CREATE", @created_by);`);
}
